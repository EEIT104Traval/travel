...
使用方法说明：
    ...
    5.关闭提示框
        myPop.closePopup();
    6.添加框低按钮
        myPop.addButton(btnName,callback); //需要注意的是添加的按键过多（弹框的宽不足容纳所有按键）时，
        将自动适应设置弹框的宽来容纳所有按键
        btnName:按键的名称,注：添加的名称含有“关闭， $("input").css("background-color","#D6D6FF"); 
        $("input").css("background-color","#D6D6FF"); $("input").css("background-color","#D6D6FF");
         $("input").css("background-color","#D6D6FF"); $("input").css("background-color","#D6D6FF");
          $("input").css("background-color","#D6D6FF");”等的将带有点击关闭弹框的事件，
    ...
    $.extend({
        initPopup: function (cfg) {
            ...
            var lzyPopup = function (cfg) {
                ...
                this.popupObj = $("<div class='lzy_custom_popup'><div class='lzy_popup_nav'>" +
//                		"<div class='lzy_nav_icon'>!</div><div class='lzy_nav_title'>消息</div>" +
                		"<svg class='lzy_nav_close'><rect width='18' height='18'/><line x1='4' y1='4' x2='14' y2='14'/><line x1='4' y1='14' x2='14' y2='4'/></svg><div class='lzy_nav_full'></div></div><div class='lzy_popup_cont'></div><div class='lzy_popup_footer'></div></div>").appendTo("body");
                ...
                setStyle: function (cfg, status) {
                    ...
                    if (status) {
                        ...
                        this.cfg = {
                            ...
                            isShowIcon: isEmpty(cfg.isShowIcon) ? false : cfg.isShowIcon,
                            icon: cfg.icon || null,
                ...
                addButton: function (btnName, callback) {
                    ...
                    var curBtn = $("<button class='lzy_footer_btn'>" + btnName + "</button>").css({borderRadius: this.cfg.borderRadius}).appendTo(this.popupObj.find(".lzy_popup_footer"))
                        .on("click", function () {
                            ...
                            if (btnName && (btnName.indexOf("关") >= 0 || btnName.indexOf("定") >= 0)) that.closePopup();//给含有“关闭”意思的按键添加关闭事件
                        })
                        .hover(function () {
                    ...
                    if (btnName && (btnName.indexOf("关") >= 0 || btnName.indexOf("定") >= 0)) curBtn.addClass("lzy_footer_btn_close");
